...
global $theme_key;
if (db_is_active()) {
  nihao_initialize_theme_settings($theme_key);
}
 ...
 */
function nihao_preprocess_maintenance_page(&$vars) {
  if (db_is_active()) {
    nihao_preprocess_page($vars);
  }
}
 ...
 * Page preprocessing
 */
function nihao_preprocess_page(&$vars) {
  ...
  $themes = nihao_theme_paths($theme_key);
  ...
  if (theme_get_setting('fix_css_limit') && !variable_get('preprocess_css', FALSE)) {
    ...
    if (nihao_css_count($css) > (30 - $style_count)) {
      $styles = '';
      $suffix = "\n".'</style>'."\n";
 ...
 * Node preprocessing
 */
function nihao_preprocess_node(&$vars) {
 ...
 */
function nihao_preprocess_comment(&$vars) {
  global $user;
 ...
 */
function nihao_preprocess_comment_wrapper(&$vars) {
 ...
 * Block preprocessing
 */
function nihao_preprocess_block(&$vars) {
  ...
  if (!isset($regions)) {
    ...
    $regions = nihao_set_regions($grid_width, $sidebar_first_width, $sidebar_last_width);
  }
  ...
  if (user_access('administer blocks', $user) && theme_get_setting('block_config_link')) {
    $vars['edit_links'] = '<div class="fusion-edit">'. implode(' ', nihao_edit_links($vars['block'])) .'</div>';
  }
}
 ...
 */
function nihao_preprocess_views_view(&$vars) {
 ...
 */
function nihao_preprocess_search_result(&$vars) {
 ...
 */
function nihao_username($object) {
 ...
 */
function nihao_grid_row($element, $name, $class='', $width='', $extra='') {
  $output = '';
  $extra = ($extra) ? ' ' . $extra : '';
  if ($element) {
    if ($class == 'full-width') {
...
function nihao_grid_block($element, $name) {
  $output = '';
  if ($element) {
 ...
 */
function nihao_set_regions($grid_width, $sidebar_first_width, $sidebar_last_width) {
 ...
 */
function nihao_edit_links($block) {
 ...
 */
function nihao_css_count($array) {
  $count = 0;
  foreach ($array as $item) {
    $count = (is_array($item)) ? $count + nihao_css_count($item) : $count + 1;
  }
  return $count;
}
 ...
 */
function nihao_theme_paths($theme) {
  $all_parents = array();
  $themes = list_themes();
